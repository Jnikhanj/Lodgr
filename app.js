...
function openPanel(kind){
  ...
  const panels={
    appearance:["Appearance",`<div class="panel-body-list"><label class="panel-row"><span>Theme</span><select id="pTheme"><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select></label><label class="panel-row"><span>Accent</span><select id="pAccent">${colors.map(c=>`<option value="${c}">${title(c)}</option>`).join("")}</select></label><label class="panel-row"><span>Financial year</span><select id="pTaxYear"><option value="2025–26">2025–26</option></select></label><label class="panel-row"><span>Start screen</span><select id="pStart"><option value="home">Home</option><option value="log">Claims</option><option value="tax">Tax</option><option value="profile">Profile</option></select></label><label class="panel-row"><span>Currency decimals</span><input id="pDecimals" type="checkbox" ${s.currencyDecimals!==false?"checked":""}/></label></div><div class="panel-actions"><button class="primary" type="button" data-save-appearance>Save</button></div>`],
...
function registerServiceWorker(){if("serviceWorker" in navigator){navigator.serviceWorker.register("sw.js?v=5.0.0").catch(()=>{});}}
